  ...
  <div class="work-management">
    <el-card>
      <template #header>
        <div class="header">
          <span>作品管理</span>
          <el-select v-model="selectedCompetitionId" placeholder="请选择竞赛" @change="loadData" style="width: 300px; margin-right: 10px;" clearable>
...
const loadCompetitions = async () => {
  try {
    ...
    if (res.data && res.data.records) {
      // 只显示需要提交作品的竞赛
      competitionList.value = res.data.records.filter(c => c.needWork === 1 && c.deleted !== 1)
    } else if (Array.isArray(res.data)) {
      competitionList.value = res.data.filter(c => c.needWork === 1 && c.deleted !== 1)
    }
  } catch (error) {
